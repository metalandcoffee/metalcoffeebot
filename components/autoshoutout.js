  ...
  for (const streamer in soList) {
    ...
    if (streamer.toLowerCase() === currentChatter.toLowerCase()) {
      ...
      if (!shoutoutsGiven.includes(streamer.toLowerCase())) {
        ...
        tmi.say(channel, `Metalhead VIP alert 🚨 @${tags['display-name']} is here and deserves your follow! They were last streaming ${channelInfo.data[0].game_name}. https://twitch.tv/${tags.username} 🌟`);
